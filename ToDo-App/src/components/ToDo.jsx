...
const addTask = () =>{
    ...
    const taskCompleted = (index) =>{
        const newTasks = tasks.map((task, i) =>{//6
            if(i == index){//7
    ...
    return (
        ...
        <p className="text-xl text-left font-semibold ">What you have for today!</p>
        <DateComp />
        </div>
<div className=" flex items-center justify-center ">
        ...
        className="font-extrabold bg-orange-700 rounded-md w-10 h-10 m-4 flex items-center justify-center text-white"><MdAdd /></button>
        </div>
...
<div className="p-2">
        <ul className="space-y-2">
            ...
            className="flex justify-between gap-5 items-center p-1 border rounded">
                {/* checkbox  */}
                <input type="checkbox" checked = {task.completed} onChange={() =>taskCompleted(index)} 
                ...
                <button onClick={() => deleteTask(index)} 
                    className="bg-orange-700 text-white p-1 rounded flex items-center"><MdDeleteOutline /></button>
